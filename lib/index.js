...
import { command, cmdVersion, git, gitEnv, loadRegistry, saveRegistry, copyTree, detectStableHome, normalizePluginSpec, preparePnpm, safeId, sandboxDir, dshHomeDir, profileDir } from './core.js'
...
async function status() {
  ...
  return { root: ROOT, stableHome: detectStableHome(), locales: localeManifest(), localeMetadata: localeMetadataManifest(), git: await cmdVersion(process.platform === 'win32' ? 'git.exe' : 'git'), sandboxes }
}
...
async function createSandbox({ id } = {}) {
  ...
  if (!stableHome || !existsSync(join(stableHome, 'profiles', 'web'))) throw new Error('STABLE DSH_HOME was not found. Set DSH_PLUGIN_SANDBOX_STABLE_HOME or use the standard ~/dsh-env/{home,home-test} layout.')
  mkdirSync(dir, { recursive: true })
  await copyTree(stableHome, dshHomeDir(ROOT, sandboxId), new Set(['node_modules', '.git']))
  await git(dir, ['init'])
  await git(dir, ['config', 'user.name', 'DSH Plugin Sandbox'])
  await git(dir, ['config', 'user.email', 'sandbox@localhost'])
  const data = registry()
  data.sandboxes[sandboxId] = { id: sandboxId, path: dir, dshHome: dshHomeDir(ROOT, sandboxId), profilePath: profileDir(ROOT, sandboxId), createdAt: now(), updatedAt: now(), source: 'stable' }
  save(data)
  await snapshot(sandboxId, 'Create sandbox from STABLE')
  return getSandbox(sandboxId)
}
async function dshExecutable(cwd) {
  const override = process.env.DSH_PLUGIN_SANDBOX_DSH_BIN
  if (override && existsSync(override)) return override
  const npm = process.platform === 'win32' ? 'npm.cmd' : 'npm'
  const prefix = String((await command(npm, ['prefix', '-g'], cwd)).stdout).trim()
  const candidate = process.platform === 'win32' ? join(prefix, 'dsh.cmd') : join(prefix, 'bin', 'dsh')
  if (!existsSync(candidate)) throw new Error(`Global dsh executable not found: ${candidate}`)
  return candidate
}
...
async function installPlugin(id, spec, sourceCwd = process.cwd()) {
  ...
  preparePnpm(cwd)
  const dsh = await dshExecutable(cwd)
  const env = { ...process.env, DSH_HOME: s.dshHome }
  const args = ['plugin', '--profile', 'web', 'add', normalized]
  try { await command(dsh, args, cwd, { env }) }
  catch (firstError) {
    await command(pnpmExecutable(), ['approve-builds', '--all'], cwd, { env }).catch(() => {})
    await command(pnpmExecutable(), ['rebuild', 'node-pty'], cwd, { env }).catch(() => {})
    await command(pnpmExecutable(), ['install', '--frozen-lockfile'], cwd, { env }).catch(() => {})
    try { await command(dsh, args, cwd, { env }) }
    catch (secondError) { throw new Error(`Plugin install failed after automatic build approval: ${secondError.stderr || firstError.stderr || secondError.message}`) }
  }
  await command(pnpmExecutable(), ['approve-builds', '--all'], cwd, { env }).catch(() => {})
  await command(pnpmExecutable(), ['rebuild', 'node-pty'], cwd, { env }).catch(() => {})
...
async function startSandbox(id) {
  ...
  const dsh = await dshExecutable(s.path)
  const env = { ...process.env, DSH_HOME: s.dshHome }
  const result = await startRuntime({ command: dsh, args: ['web', '--host', '127.0.0.1', '--port', '0', '--no-open'], cwd: s.path, env })
...
async function validateSandbox(id) {
  ...
  try { preparePnpm(s.profilePath); await command(pnpmExecutable(), ['install', '--frozen-lockfile'], s.profilePath, { env: { ...process.env, DSH_HOME: s.dshHome } }); add('Frozen pnpm install', true) } catch (e) { add('Frozen pnpm install', false, String(e.stderr || e.message)) }
  if (checks.every(c => c.ok)) {
    try { const r = await startSandbox(id); add('DSH sandbox boot', true, r.url); await stopSandbox(id) }
...
async function promote(id) {
  ...
  const backup = `${stable}.dsh-plugin-sandbox-backup-${Date.now()}`
  await copyTree(stable, backup, new Set(['node_modules', '.git']))
  try { await copyTree(s.dshHome, stable, new Set(['node_modules', '.git'])) } catch (e) { throw new Error(`Promotion failed; backup preserved at ${backup}: ${e.message}`) }
  return { ok: true, stable, backup }
}
