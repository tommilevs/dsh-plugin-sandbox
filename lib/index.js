...
import { fetchLatestRelease, installVerifiedRelease, releaseStatus as getReleaseStatus, verifyChecksum } from './releases.js'
...
const RELEASE_REPOSITORY = 'tommilevs/dsh-plugin-sandbox'
...
async function pluginReleaseStatus() {
  const fallback = JSON.parse(readFileSync(PACKAGE_PATH, 'utf8')).version
  const stableHome = detectStableHome()
  const profile = stableHome && join(stableHome, 'profiles', 'web')
  let installedSpec = '', installedVersion = fallback
  if (profile && existsSync(join(profile, 'package.json'))) {
    const manifest = JSON.parse(readFileSync(join(profile, 'package.json'), 'utf8'))
    installedSpec = manifest.dependencies?.['dsh-plugin-sandbox'] || ''
    const installed = join(profile, 'node_modules', 'dsh-plugin-sandbox', 'package.json')
    if (existsSync(installed)) installedVersion = JSON.parse(readFileSync(installed, 'utf8')).version || fallback
  }
  try {
    return await getReleaseStatus({ installedSpec, installedVersion, fetchRelease: () => fetchLatestRelease(fetch, RELEASE_REPOSITORY), repo: RELEASE_REPOSITORY })
  } catch (error) {
    return { state: 'unavailable', installedVersion, error: redactDiagnostic(error.message) }
  }
}

async function updatePluginRelease() {
  const stableHome = detectStableHome()
  const profile = stableHome && join(stableHome, 'profiles', 'web')
  if (!profile) throw new Error('Main DSH profile was not found.')
  const status = await pluginReleaseStatus()
  if (status.state === 'development-link') throw new Error('A linked development checkout will not be replaced.')
  if (status.state !== 'update-available') throw new Error('No verified plugin update is available.')
  const [archiveResponse, checksumResponse] = await Promise.all([fetch(status.release.archiveUrl), fetch(status.release.checksumUrl)])
  if (!archiveResponse.ok || !checksumResponse.ok) throw new Error('Release download failed.')
  const archive = Buffer.from(await archiveResponse.arrayBuffer())
  verifyChecksum(archive, await checksumResponse.text())
  const archivePath = join(ROOT, 'releases', `dsh-plugin-sandbox-${status.release.version}.tgz`)
  mkdirSync(dirname(archivePath), { recursive: true })
  writeFileSync(archivePath, archive, { mode: 0o600 })
  const runtime = await resolveRuntime()
  return installVerifiedRelease({ profile, archivePath, version: status.release.version, install: () => installProfile(runtime, profile, stableHome, false) })
}
    ...
    const handler = async (req, res) => {
      try {
        ...
        switch (method) {
          ...
          case 'release-status': result = await pluginReleaseStatus(); break
          case 'update-release': result = await updatePluginRelease(); break
