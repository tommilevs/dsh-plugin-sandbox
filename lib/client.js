...
function Button({ children, onClick, disabled = false, title = undefined }) {
  return React.createElement('button', {
    type: 'button', disabled, onClick, title,
    style: {
...
function AddLanguageDialog({ open, onClose }) {
  ...
  async function installLocal() {
    ...
    try {
      ...
      const untranslatedKeys = localeNeedsTranslation(values, migrationKeys)
  ...
  async function submitGithub() {
    ...
    try {
      ...
      const untranslatedKeys = localeNeedsTranslation(values, [...sourceUntranslated])
      if (untranslatedKeys.length) throw new Error(formatMessage(t('messages.translateFirst'), { count: untranslatedKeys.length }))
  ...
  const draftMissingKeys = missingLocaleKeys(draftValues)
  const draftUntranslatedKeys = localMatch
    ? [...new Set([...(localMatch.untranslatedKeys || []), ...draftMissingKeys])].filter(key => key in EN && draftValues?.[key] === EN[key])
    : []
  const untranslatedKeys = draftUntranslatedKeys
  ...
  const submitBlockedByTranslation = untranslatedKeys.length > 0
  const remoteMessage = (() => {
    ...
    if (remoteState.state === 'ready-to-submit') return submitBlockedByTranslation
      ? `🟡 ${formatMessage(t('messages.translateFirst'), { count: untranslatedKeys.length })}`
  ...
  const submitLabel = submitBlockedByTranslation
    ? `🟡 ${formatMessage(t('messages.translateFirst'), { count: untranslatedKeys.length })}`
    : localMatch
  ...
  return React.createElement('div', { style: { position: 'fixed', inset: 0, zIndex: 10001, background: 'rgba(0,0,0,.56)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 } },
    React.createElement('div', { style: { width: 'min(920px, 96vw)', maxHeight: '92vh', overflow: 'auto', border: '1px solid var(--dsw-alias-border-subtle, #555)', borderRadius: 16, background: 'var(--dsw-alias-surface-primary, #111)', padding: 20, boxShadow: '0 25px 80px rgba(0,0,0,.5)' } },
      ...
      submitBlockedByTranslation && React.createElement('div', { style: { marginTop: 8, fontSize: 12, opacity: .92 } },
        React.createElement('div', null, `🟡 ${formatMessage(t('messages.translationProgress'), { translated: translatedTranslationKeys, total: totalTranslationKeys, percent: translationPercent })}`),
      ...
      React.createElement('div', { style: { display: 'flex', justifyContent: 'flex-end', flexWrap: 'wrap', gap: 8, marginTop: 14 } },
        ...
        React.createElement(Button, { onClick: submitGithub, disabled: busy || remoteState?.state === 'published' || submitBlockedByTranslation, title: submitBlockedByTranslation ? formatMessage(t('messages.translateFirst'), { count: untranslatedKeys.length }) : undefined }, submitLabel),
      ),
    ),
  )
}
...
function SandboxPanel() {
  ...
  const refresh = React.useCallback(async () => {
    try {
      ...
      mergeCustomLocales(data.locales || {})
  ...
  async function create() {
    const id = window.prompt(t('dialogs.sandboxName'), `sandbox-${Date.now()}`)
    if (id) await withRefresh(() => rpc('create', { id }))
  }
  ...
  async function promote() {
    ...
    try {
      ...
      window.alert(`${t('messages.promoted')}\nBackup: ${r.backup}`)
      await refresh()
    } catch (e) { window.alert(String(e.message || e)) }
  }
  ...
  return React.createElement(React.Fragment, null,
    ...
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 16 } },
      ...
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
        ...
        React.createElement(Button, { onClick: create }, t('actions.newSandbox')),
      ),
    ),
